...
namespace Tierverwaltung.Tiertypen
{
    internal class Elefant : Tier
    {
        ...
        private const double MinimalGewichtElefant = 150;
        ...
        public Elefant(string name, double gewicht) : base(name, gewicht)
        {
            ...
            Thread.Sleep(1000);
            if (gewicht < MinimalGewichtElefant) 
            {
                Console.ForegroundColor = ConsoleColor.DarkRed;
                Console.WriteLine($"{name} hat {gewicht} Kg, weniger als {MinimalGewichtElefant} Kg. Das Tier scheint krank zu sein.");
            }
            Console.ForegroundColor = ConsoleColor.DarkYellow;
            Console.WriteLine($"Mit dem Elefanten {name} / {gewicht} kg haben wir {tierZaehler} Elefanten und insgesamt {TotalTierZaehler} Tiere.\n\n");
        }
