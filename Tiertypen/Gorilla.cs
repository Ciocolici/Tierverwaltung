...
namespace Tierverwaltung.Tiertypen
{
    internal class Gorilla : Tier
    {
        ...
        private const double MinimalGewichtGorilla = 100;
        ...
        public Gorilla(string name, double gewicht) : base(name, gewicht)
        {
            ...
            Thread.Sleep(1000);
            if (gewicht < MinimalGewichtGorilla)
            {
                Console.ForegroundColor = ConsoleColor.DarkRed;
                Console.WriteLine($"{name} hat {gewicht} Kg, weniger als {MinimalGewichtGorilla} Kg. Das Tier scheint krank zu sein.");
            }
            Console.ForegroundColor = ConsoleColor.DarkYellow;
            Console.WriteLine($"Mit dem Gorilla {name} / {gewicht} kg haben wir {tierZaehler} Gorillas und insgesamt {TotalTierZaehler} Tiere.\n\n");
        }
